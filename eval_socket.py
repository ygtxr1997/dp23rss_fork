...
yes | CUDA_VISIBLE_DEVICES=6 python eval_socket.py  \
    -c "None"  \
...
def main(checkpoint, output_dir, device, from_config='', port=6060, domain_shift='none',
    ...
    for idx in range(repeat_times):
        ...
        )  # {'test/mean_score': 0.123, 'test/sim_max_reward_0': 0.5, ...}
        if not close_online:
        ...
        # Better success metric: count per-env max reward from runner output
        # (`test/sim_max_reward_{seed}` generated by PushTImageSocketRunner.run).
        per_env_rewards = [
            float(v) for k, v in json_log.items()
            if k.startswith("test/sim_max_reward_")
        ]
        assert len(per_env_rewards) > 0, "Missing `test/sim_max_reward_*` in runner log."

        this_success = int(np.sum(np.array(per_env_rewards) >= success_threshold))
        this_total = len(per_env_rewards)
        success_count += this_success
        success_total += this_total
        ...
        success_rate = success_count / success_total if success_total > 0 else 0.0
        print(f"[shift:{domain_shift}-seed:{env_runner_config.test_start_seed}"
              ...
              f"suc_this={this_success}/{this_total}, "
              f"suc_all({success_threshold:.2f})={success_count}/{success_total}({success_rate:.1%})")
    ...
    final_success_rate = success_count / success_total if success_total > 0 else 0.0
    print(
        f"[Final]: mean={eval_mean:.4f}, std={eval_std:.4f}, "
        f"suc({success_threshold:.2f})={success_count}/{success_total}({final_success_rate:.1%})"
    )
